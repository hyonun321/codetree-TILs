...
def dou_roll():
    ...
    while True :
        ...
        if cut_index+(row) >= n : break # 다음번껄 미리 예상해서 재본다.
...
def dou_push():
    ...
    pushed = [[[ ]for _ in range(n)] for _ in range(n)] # 방향 visited 체크 하는 3차원 배열
...
def check_mill_max_min_k(k):
    if is_maps() <= k: return False
    return True
